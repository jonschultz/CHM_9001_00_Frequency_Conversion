...
* The two large capacitors are electrolytics, and should be rated for at least 200V. On my board, I've implemented C1 (the flying capacitor) with two 220 microfarad, 200V capacitors in parallel.<br>
...
* While the "PMOS1" FET using SQJ431EP runs reliably, it was definitely the most fragile component during the design and development of this supply. I've blown up quite a few of these, which tend to fail short, but I don't know exactly what the root cause was in each instance. The current rating for these devices is just a bit more than what they need to handle in operation, and P-channel MOSFETs, in general, are slightly less capable than their N-channel counterparts. The FET at this position is also quite critical, because it switches current on the high side between the flying cap and the storage cap. Depending on the state of the system, the inrush current can be high. R10 is intended to significantly limit the inrush current, but it also makes the system less efficient. If this device (or any of the switching FETs) fails short, the supply will still function but the isolation will be compromised, and your GFCI will trip. Substituting SQJ431EP with a P-channel MOSFET with higher specs could be good design change, possibly allowing for a smaller valued R10, but P-channel MOSFETs get expensive quickly. Anyone reproducing this circuit might consider beefier options, possibly a through-hole device. Even better would be to redesign for N-channel, but driving it becomes more complicated.<br>
...
* There are definitely more efficient ways of building a 120V DC supply using 120V AC input, such as with a switching power supply architecture. The complexity of such designs is currently outside the realm of what I can currently handle myself, so instead I adapted concepts from some previous projects. It was a curiosity and challenge, simply to see if I could make such a circuit work. I've been happy with performance and reliability, but I think there are definitely improvements that could be made. If you use or adapt this design, please do so considering the safety risks, to you and your equipment!<br>
